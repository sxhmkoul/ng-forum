...
export class AuthService {
  API_KEY = 'AIzaSyBPKqRl2tKhg-6UcFRQsjuUAgNmqva_tq0';
  ...
  createUser = (signupForm : any) => {
    return this.http.post('https://ng-backend-8a3aa-default-rtdb.firebaseio.com/users', {
      params: {
                  displayPicture : signupForm.dp,
                  name: signupForm.name,
                  profession: signupForm.job,
  ...
  signup = (userData: AuthModel["signUpPayload"]) => {
   return this.http.post<AuthModel['signUpResponse']>('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key='+this.API_KEY,{
  ...
  login = (loginData: AuthModel["loginPayload"]) => {
    return this.http.post<AuthModel['loginResponse']>('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key='+this.API_KEY,{
